...
- `docs/001679511(3).xlsx` — 厚生劳动省原版「年度更新申告書計算支援ツール（継続事業用）」Excel，是本项目的对照源文件（只读，勿修改），**同时是 Excel 导出的运行时模板**（.dockerignore 已再包含、`outputFileTracingIncludes` 已追踪进 standalone）。
...
./scripts/dev-db.sh     # 启动本地 MariaDB（docker，数据在命名卷 asco-tools_mariadb_data）；
                        # 若 .env.local 的 DATABASE_URL 指向远程 dev 库则可跳过
pnpm db:migrate
...
- 不使用 docker compose；`scripts/dev-db.sh` 与 `scripts/run-app.sh` 用纯 docker 命令管理容器，二者共享 docker 网络 `asco-tools`，使用本地库时应用容器内以主机名 `asco-tools-mariadb` 连接数据库。
- `run-app.sh` 会自动读取并转发 `.env.local`（gitignore，不入库）进容器；远程 dev 库的连接串就配置在该文件的 `DATABASE_URL`。优先级：外部环境变量 > `.env.local` > 本地容器回退值。
...
- **功能开关**（`src/lib/feature-flags.ts` + `feature_flag` 表）：`assistant`（AI 申告助手）与 `billing`（回数パック购买）由管理员在 `/admin` 的「機能スイッチ」页实时切换；OFF 时用户界面隐藏对应功能（购买入口显示「準備中」占位），API 侧同时拒绝（assistant/checkout 404）。默认全 OFF，表未迁移时按全 OFF 降级。新增受控功能时在 `featureFlagDefs` 登记并在 UI/API 两侧接线。

### 计费：预付导出积分（非订阅）

注意：README 中「`user.plan === "pro"` 校验」的说法已过时，实际实现是积分制（`docs/stripe-integration-plan.md` 为准）：

- 注册赠送 `FREE_EXPORT_CREDITS`（默认 3）次导出；**每份申告书只扣 1 积分**：首次导出（任意格式）扣减，之后同一申告书的 Excel/PDF 重复导出免费（以 `export_log` 中该 declarationId 的既有记录为凭据，判定在 FOR UPDATE 事务内完成）。
...
- Excel 导出（`src/lib/official-template.ts`）把申告数据**写入厚労省原版 Excel 模板**后返回：ExcelJS 无法往返读写该复杂文件，因此用 jszip 直接编辑 xlsx 内的工作表 XML（只填输入单元格、不碰公式，`fullCalcOnLoad` 让 Excel 打开时重算）。输入单元格地址映射写在该文件常量中，模板换年度版本时需重新核对。PDF（`src/lib/exporters.ts`）用 PDFKit + Noto Sans JP。导出 API（`src/app/api/export/*`）在服务端校验登录并扣积分，402 表示积分不足。
