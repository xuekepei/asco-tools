...
const wageGroups: { countKey: keyof MonthEntry; wageKey: keyof MonthEntry; label: string; helpKey: string }[] = [
  { countKey: "regularWorkers", wageKey: "regularWages", label: "常用", helpKey: "regularWorkers" },
  { countKey: "officerWorkers", wageKey: "officerWages", label: "役員", helpKey: "officerWorkers" },
  { countKey: "temporaryWorkers", wageKey: "temporaryWages", label: "臨時", helpKey: "temporaryWorkers" },
  { countKey: "insuredWorkers", wageKey: "insuredWages", label: "雇用被保険者", helpKey: "insuredWorkers" },
...
export function DeclarationWorkspace({ user, aiEnabled, showAssistant = false, isAdmin, initialView = "list", initialSection = "business", initialDeclaration = null }: { user: { name: string; email: string }; aiEnabled: boolean; showAssistant?: boolean; isAdmin: boolean; initialView?: WorkspaceView; initialSection?: Section; initialDeclaration?: { id: string; formData: unknown } | null }) {
  ...
  const copyPreviousMonth = (index: number) => {
    if (index === 0) return;
    editVersion.current += 1;
    setDirty(true);
    setData((current) => ({
      ...current,
      months: current.months.map((month, monthIndex) =>
        monthIndex === index ? { ...current.months[index - 1], month: month.month } : month,
      ),
    }));
  };

  const applyFirstMonthToAll = () => {
    if (!window.confirm("4月の人数・賃金をすべての月にコピーします。既に入力した5月以降の値は上書きされます。よろしいですか？")) return;
    editVersion.current += 1;
    setDirty(true);
    setData((current) => ({
      ...current,
      months: current.months.map((month, monthIndex) =>
        monthIndex === 0 ? month : { ...current.months[0], month: month.month },
      ),
    }));
  };
  ...
  const exportFile = async (format: "excel" | "pdf") => {
    setNotice("");
    if (!activeId) {
      setNotice("保存後にエクスポートしてください");
      return;
    }
    const response = await fetch(`/api/export/${format}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ declarationId: activeId, declaration: data }),
    });
...
function CreditPaywall({ configured, packs, loading, onCheckout, onClose }: { configured: boolean; packs: BillingInfo["packs"]; loading: BillingInfo["packs"][number]["key"] | "month" | "year" | "portal" | null; onCheckout: (pack: BillingInfo["packs"][number]["key"]) => void; onClose: () => void }) {
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-[#112019]/55 p-4 backdrop-blur-sm"><div className="w-full max-w-lg rounded-[28px] bg-white p-7 shadow-2xl"><div className="flex justify-between"><span className="grid size-12 place-items-center rounded-2xl bg-[#e8f3e9] text-[#216249]"><Sparkles size={23} /></span><button onClick={onClose}><X size={20} className="text-[#77837b]" /></button></div><h2 className="mt-6 text-2xl font-semibold tracking-tight">出力回数を購入</h2><p className="mt-3 leading-7 text-[#68756d]">登録時の無料回数を使い切りました。月額・年額契約はなく、購入した回数に有効期限はありません。</p>{configured ? <><div className="mt-6 grid gap-3 sm:grid-cols-3">{packs.map((pack) => <button key={pack.key} className="button-secondary h-14" disabled={!pack.available || loading !== null} onClick={() => onCheckout(pack.key)}>{loading === pack.key && <LoaderCircle className="animate-spin" size={16} />}{pack.credits}回</button>)}</div><p className="mt-3 text-center text-xs text-[#8a948d]">安全なStripe Checkoutへ移動します</p></> : <div className="mt-6 rounded-2xl border border-dashed border-[#d3dbd3] bg-[#fafbf8] p-5 text-center"><span className="rounded-full bg-[#eef1ed] px-3 py-1 text-xs font-semibold text-[#5f6c64]">COMING SOON</span><p className="mt-3 font-semibold text-[#4c5a51]">回数パックの購入は準備中です</p><p className="mt-2 text-sm leading-6 text-[#77837b]">提供開始までしばらくお待ちください。</p></div>}</div></div>;
}
